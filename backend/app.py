...
class_labels = ['Benignware', 'Unknown', 'Hackware', 'Malware']  # Replace with your actual class names
...
def firmware_analysis_table(analysis_data):
    ...
    for key, value in analysis_data.items():
        ...
        if isinstance(value, list):
            if len(value) > 0:
                value = '<br>'.join(value)  # Join list items with <br> for proper HTML display
            else:
                value = 'None found'  # If list is empty, explicitly say no data
        
        # Check if the value is a dictionary (e.g., metadata)
        elif isinstance(value, dict):
            value = '<br>'.join([f'{k}: {v}' for k, v in value.items()])  # Convert dict to key-value HTML format
...
def predict():
    ...
    img_array = np.repeat(img_array, 3, axis=-1)  # Convert to 3 channels (RGB-like for model compatibility)
